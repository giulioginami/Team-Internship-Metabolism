%% Generate_VirtualPopulation_v2.m
% STEP 3 of the data-improvement plan. Identical to
% Generate_VirtualPopulation.m EXCEPT the LHS bounds for the three
% physiological-input priors are re-tightened to the REAL Japan ranges
% measured in step 2 (Analyze_Japan_Features.m, jf.marginal):
%
%   prior    old bound     new bound     reason (Japan p5..p95, all cats)
%   ------   -----------   -----------   ----------------------------------
%   G_b      3.9 .. 12.0   4.0 .. 8.0    real ~4.1..7.7, even T2DM p95=7.65
%   I_PL_b   2.0 .. 80.0   1.5 .. 25.0   real ~1.5..24  (cohort not hyperins.)
%   BW      60.0 .. 130.0  45.0 .. 105.0 lean Japanese cohort, median ~60
%
% Kinetic priors (k1,k5,k6,k8) are LEFT UNCHANGED (no Japan ground truth
% for latent kinetics; user decision: "fix bounds only").
% Sampling stays uniform LHS (user decision).
%
% NOTE / leakage caveat: these bounds are tuned on the same Japan cohort
% later used to validate the architecture (user chose "use all Japan").
% Architecture metrics on Japan are therefore optimistic and must be
% reported as such.
%
% The original Generate_VirtualPopulation.m and its outputs are left
% intact so step 4 can compare PREVIOUS vs NEW synthetic data.
% Output: virtual_population_v2.mat
%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%
...
% >>> Rows 5-7 (G_b, I_PL_b, BW) re-prioredd from Japan step-2 marginals <<<
% =========================================================================
param_bounds = [
    0.02,    0.03;   % k1  [1/min]      (unchanged - latent kinetic)
    0.0,    0.17;    % k5  [1/min]      (unchanged - latent kinetic)
    0.0,    0.34;    % k6  [-]          (unchanged - latent kinetic)
    0.0,    10.0;    % k8  [-]          (unchanged - latent kinetic)
    4.0,    8.0;     % G_b [mmol/L]     (was 3.9..12.0  -> Japan-real)
    1.5,    25.0;    % I_PL_b [mU/L]    (was 2.0..80.0  -> Japan-real)
    45.0,   105.0;   % BW  [kg]         (was 60.0..130.0 -> Japan-real)
];
...
% =========================================================================
fprintf('Starting virtual population generation v2 (N=%d)...\n', N);
...
virtual_population.param_bounds   = param_bounds;    % record what we used
virtual_population.version        = 'v2_japan_repriored';

save('virtual_population_v2.mat', 'virtual_population', '-v7.3');
fprintf('Saved: virtual_population_v2.mat\n');
...
figure('Name','Virtual Population v2 - Overview','Position',[100 100 1200 500]);
...
sgtitle(sprintf('EDES Virtual Population v2 (Japan-repriored)  |  N_{valid} = %d / %d', n_valid, N));
