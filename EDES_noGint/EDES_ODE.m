function dxdt = EDES_ODE(t,variables,parameters,constants,input_data,individual)
%% Implementation of the Eindhoven Diabetes Education Simulator 
%(Rozendaal et al. (2018) model of postprandial glucose and insulin dynamics 
%G_int term has been removed
%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%
%Inputs
%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%
%t          - time value
...
%individual - individual (row of input data array) being simulated
%% %%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%
...
I_PL = variables(3); %plasma insulin concentration (mU/l)
I_d1 = variables(4); % delayed/remote insulin concentration 
...
%% model input
D_meal_G = input_data.meal.G(individual); % total amount of carbohydrates ingensted (mg)
BW = input_data.BW(individual); %body weight (kg)
...
I_pnc = (f_I.^-1).*(k6.*(G_PL - G_b) + (k7/tau_i).*G_b + (k8.*tau_d).*D.G_PL);
%insulin  liver
...
dxdt=[D.M_G_gut;D.G_PL;D.I_PL;D.I_d1];