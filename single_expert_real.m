%% single_expert_real.m
% Fits the single-expert EDES model to sparse OGTT data from one real patient
% in japan_population_labelled.mat, using Shauna's optimisation approach
% (Latin Hypercube multi-start, 3 free parameters [k1, k5, k6]).
%
% Mirror of final_architecture_real.m but with single-expert instead of MoE:
%   1. Optimise [k1, k5, k6] via LHS multi-start lsqnonlin on EDES_ErrorFunc
%   2. Plot: fitted EDES curve (solid) + observed data
%
% Optimisation settings (same as single_expert_dataset.m):
